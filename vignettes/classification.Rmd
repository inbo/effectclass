...
The upper threshold devides the zone of a `positive effect`.
...
We split the `negative effect` in a `strong negative effect`, `moderate negative effect` and `negative effect` using the simular rules as for the `positve effect`.
...
The table below givens an overview of the ten classes that we can distinct based on the reference, lower and upper threshold.
...
| `-`    | negative effect | decrease | confidence interval below reference and contains the lowwer threshold | 1 |

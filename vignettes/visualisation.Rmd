...
In such case we recommend to add `geom = "rect"` or `geom = "linerange"` to `stat_fan` over using errorbars.
...
ggplot(ds, aes(x = effect, y = mean, ymin = lcl, ymax = ucl, link_sd = sd)) +
  ...
  scale_effect(detailed = FALSE, signed = FALSE)
```
...
E.g. sort first on the the classification and then on the estimate.
...
The table below gives an example on how then estimate and its confidence interval gets translated into a classification and formatted text.
